...
(function () {
    ...
    function authoriseGoogle() {
        return new Promise(async (resolve, reject) => {
            ...
            client.callback = (resp) => {
                ...
                _tokenExpiry =
                    Date.now() +
                    (resp.expires_in ? resp.expires_in * 1000 : 3600_000);
                resolve(_accessToken);
            };

            // Prompt only if necessary
            client.requestAccessToken({
                prompt: _accessToken ? '' : 'select_account',
            });
        });
    }
    ...
    function validateDiagram(data) {
        ...
        if (typeof data.version === 'undefined')
            throw new Error('Missing version field — not a diagram file');
        if (!Array.isArray(data.tabs) && typeof data.nodes === 'undefined') {
            throw new Error(
                'File does not contain diagram data (no tabs or nodes)',
            );
        }
    }
  ...
  `;
    ...
    `;
        ...
        filenameInput.setSelectionRange(
            0,
            filenameInput.value.lastIndexOf('.'),
        );
    ...
    function dialogKeyHandler(e) {
        ...
        if (
            ...
            document.activeElement.id !== 'gd-filename'
        )
            confirmSave();
    }
    ...
    function renderBreadcrumb() {
        ...
        if (!bc) return;
        bc.innerHTML = dialogState.stack
            .map((seg, i) => {
    ...
    async function loadFolders() {
        ...
        if (list)
            list.innerHTML = `<div class="gd-loading-msg"><div class="gd-spinner"></div> Loading…</div>`;
        ...
        try {
            ...
            if (list)
                list.innerHTML = `<div class="gd-empty-msg">Failed to load folders</div>`;
            showError(e.message);
        } finally {
    ...
    async function saveToGoogleDrive() {
        ...
        const diagramName =
            (editorState && editorState.diagramName) || 'diagram';
            ...
            showSaveToDriveDialog(
                ...
                async ({ filename, folderId }) => {
                    ...
                    try {
                        ...
                        // Update editor state
                        if (editorState) {
                            editorState.diagramName = filename.replace(
                                /\.json$/i,
                                '',
                            );
                            editorState.dirty = false;
                            if (
                                typeof window._updateTitleDisplay === 'function'
                            )
                                window._updateTitleDisplay();
                            if (
                                typeof window._saveToLocalStorage === 'function'
                            )
    ...
    `;
        ...
        document.getElementById('gd-open-cancel').onclick = () =>
            closeOpenDialog(false);
        document.getElementById('gd-open-btn').onclick = confirmOpen;
        overlay.addEventListener('click', (e) => {
            if (e.target === overlay) closeOpenDialog(false);
        });
    ...
    function renderOpenBreadcrumb() {
        ...
        if (!bc) return;
        bc.innerHTML = openDialogState.stack
            .map((seg, i) => {
    ...
    async function loadItems() {
        ...
        if (list)
            list.innerHTML = `<div class="gd-loading-msg"><div class="gd-spinner"></div> Loading…</div>`;
        const currentId =
            openDialogState.stack[openDialogState.stack.length - 1].id;

        try {
            const items = await listDriveItems(
                currentId,
                openDialogState.token,
            );
            openDialogState.items = items;
            renderItemList(items);
        } catch (e) {
            if (list)
                list.innerHTML = `<div class="gd-empty-msg">Failed to load folder</div>`;
    ...
    function renderItemList(items) {
        ...
        list.querySelectorAll('.gd-file-item').forEach((el) => {
            el.onclick = () => {
                ...
                list.querySelectorAll('.gd-file-item').forEach((f) =>
    ...
    async function confirmOpen() {
        ...
        try {
            ...
            showOpenError(
                'This file is not a valid diagram — could not parse JSON.',
            );
            if (openBtn) {
                openBtn.disabled = false;
                openBtn.textContent = 'Open';
